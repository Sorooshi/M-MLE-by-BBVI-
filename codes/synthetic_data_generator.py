...
def generate_Y(N, V, K, pr_v, cardinality, features_type, V_noise1):
    ...
    :param V_noise1: Int, number of columns to insert noisy features regarding the
    ...
    if features_type == 'Q':  # quantitative
        ...
        rnd_idx_features = np.random.randint(0, V, size=int(np.ceil(V/2)))
        Y = Y[:, rnd_idx_features]
        Yn = np.concatenate((Y, noise1), axis=1)
        np.random.shuffle(Yn.T)
