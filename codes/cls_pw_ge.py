...
class ClsMcMcGE(tf.keras.Model):

    """ Classification using Monte Carlo Gradient Estimator (PW-FO) """

    def __init__(self, n_units, n_features, n_classes, dense_layer_type, activation, name='phi', **kwargs):
        super(ClsMcMcGE, self).__init__(name=name, **kwargs)
    ...
    def call(self, inputs, training=True):
        ...
        py_x_z_logits = self.py_x_z(x)
        return py_x_z_logits


def apply_cls_ge(n_units, n_features, n_classes, n_epochs, batch_size, learning_rate,
                 dense_layer_type, activation=None, x_train=None, y_train=None,
    ...
    model_adv = ClsMcMcGE(n_units=n_units, n_features=n_features, n_classes=n_classes,
    ...
    def training_step(x_batch, y_batch, model):
        with tf.GradientTape() as tape:
            py_x_z_logits = model(x_batch, training=True)
            neg_log_lik_py_x_z_cls = tf.nn.softmax_cross_entropy_with_logits(labels=y_batch, logits=py_x_z_logits)
            kl_loss = sum(model.losses)/(x_train.shape[0])

            total_loss = neg_log_lik_py_x_z_cls + kl_loss
    ...
    def testing_step(x_batch_t, y_batch_t, model):
        py_x_z_logits_t = model(x_batch_t, training=False)
        neg_log_lik_py_x_z_cls_t = -tf.reduce_sum(
            tf.nn.softmax_cross_entropy_with_logits(labels=y_batch_t, logits=py_x_z_logits_t))
        kl_loss = sum(model.losses)/(x_test.shape[0])

        total_loss_t = neg_log_lik_py_x_z_cls_t + kl_loss
...
if __name__ == '__main__':
    ...
    model_adv = apply_cls_ge(n_units=n_units, n_features=n_features, n_classes=n_classes,
    ...
    if inside_call:
        adm.plot_roc_auv_curve_of_an_algorithm(alg_ms=labels_pred, gt_ms=labels_true,
                                               ...
                                               data_name='make_cls', alg_name="cls-ge"+dense_layer_type,
